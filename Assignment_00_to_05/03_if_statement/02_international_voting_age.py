...
def main():
    ...
    VOTING_AGE_PETURKSBOIPO: int = 16
    VOTING_AGE_STANLAU: int = 25
    VOTING_AGE_MAYENGUA: int = 48

    user_age: int = int(input("How old are you? "))

    if user_age >= VOTING_AGE_PETURKSBOIPO:
        print(f"You can vote in Peturksbouipo where the voting age is {VOTING_AGE_PETURKSBOIPO}.")
    else:
        print(f"You cannot vote in Peturksbouipo where the voting age is {VOTING_AGE_PETURKSBOIPO}.")
